...
namespace GTA
{
    public sealed class WeaponComponentCollection
    {
        ...
        readonly static WeaponComponent _invalidComponent = new(null, null, WeaponComponentHash.Invalid);
        ...
        /// </returns>
        [Obsolete(
            "WeaponComponentCollection.GetLuxuryFinishComponent is wrongly named and cannot necessarily get all of the components for gun_root (e.g. camo components), please use WeaponComponentCollection.GetGunRootComponent instead.")]
        ...
        {
            return NativeMemory.GetAllCompatibleWeaponComponentHashes((uint)hash).Select(x => (WeaponComponentHash)x)
                .ToArray();
        }
    }
}