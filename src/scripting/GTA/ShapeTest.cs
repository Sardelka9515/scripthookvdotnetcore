...
namespace GTA
{
    public static class ShapeTest
    {
        ...
        /// </value>
        public static ShapeTestHandle StartTestLOSProbe(Vector3 startPosition, Vector3 endPosition,
            IntersectFlags intersectFlags = IntersectFlags.Map, Entity excludeEntity = null,
            ShapeTestOptions options = ShapeTestOptions.Default)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_LOS_PROBE, startPosition.X, startPosition.Y,
                startPosition.Z, endPosition.X, endPosition.Y, endPosition.Z, intersectFlags,
                excludeEntity == null ? 0 : excludeEntity.Handle, options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartExpensiveSyncTestLOSProbe(Vector3 startPosition, Vector3 endPosition,
            IntersectFlags intersectFlags = IntersectFlags.Map, Entity excludeEntity = null,
            ShapeTestOptions options = ShapeTestOptions.Default)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_EXPENSIVE_SYNCHRONOUS_SHAPE_TEST_LOS_PROBE, startPosition.X,
                startPosition.Y, startPosition.Z, endPosition.X, endPosition.Y, endPosition.Z, intersectFlags,
                excludeEntity == null ? 0 : excludeEntity.Handle, options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartTestBoundingBox(Entity entity,
            IntersectFlags intersectFlags = IntersectFlags.BoundingBox,
            ShapeTestOptions options = ShapeTestOptions.IgnoreNoCollision)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_BOUNDING_BOX, entity.Handle, intersectFlags,
                options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartTestBox(Vector3 sourcePosition, Vector3 dimension, Vector3 rotationAngles,
            EulerRotationOrder rotationOrder = EulerRotationOrder.YXZ,
            IntersectFlags intersectFlags = IntersectFlags.Map, Entity excludeEntity = null,
            ShapeTestOptions options = ShapeTestOptions.IgnoreNoCollision)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_BOX, sourcePosition.X, sourcePosition.Y,
                sourcePosition.Z, dimension.X, dimension.Y, dimension.Z, rotationAngles.X, rotationAngles.Y,
                rotationAngles.Z, rotationOrder, intersectFlags, excludeEntity == null ? 0 : excludeEntity.Handle,
                options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartTestBound(Entity entity, IntersectFlags intersectFlags = IntersectFlags.Map,
            ShapeTestOptions options = ShapeTestOptions.IgnoreNoCollision)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_BOUND, entity.Handle, intersectFlags, options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartTestCapsule(Vector3 startPosition, Vector3 endPosition, float radius,
            IntersectFlags intersectFlags = IntersectFlags.Map, Entity excludeEntity = null,
            ShapeTestOptions options = ShapeTestOptions.IgnoreNoCollision)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_CAPSULE, startPosition.X, startPosition.Y,
                startPosition.Z, endPosition.X, endPosition.Y, endPosition.Z, radius, intersectFlags,
                excludeEntity == null ? 0 : excludeEntity.Handle, options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartTestSweptSphere(Vector3 startPosition, Vector3 endPosition, float radius,
            IntersectFlags intersectFlags = IntersectFlags.Map, Entity excludeEntity = null,
            ShapeTestOptions options = ShapeTestOptions.IgnoreNoCollision)
        {
            return new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_SWEPT_SPHERE, startPosition.X, startPosition.Y,
                startPosition.Z, endPosition.X, endPosition.Y, endPosition.Z, radius, intersectFlags,
                excludeEntity == null ? 0 : excludeEntity.Handle, options));
        }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static ShapeTestHandle StartTestMouseCursorLOSProbe(out Vector3 probeStartPosition,
            out Vector3 probeEndPosition, IntersectFlags intersectFlags = IntersectFlags.Map,
            Entity excludeEntity = null, ShapeTestOptions options = ShapeTestOptions.Default)
        {
            ...
            unsafe
            {
                handle = new ShapeTestHandle(Call<int>(Hash.START_SHAPE_TEST_MOUSE_CURSOR_LOS_PROBE,
                    &outProbeStartPositionNative, &outProbeEndPositionNative, intersectFlags,
                    excludeEntity == null ? 0 : excludeEntity.Handle, options));
            }
        ...
        /// If this method fails to create the shape test request because there are too many ongoing requests, <see cref="ShapeTestHandle.IsRequestFailed"/> will return <see langword="true" /> on the handle struct.
        /// </value>
        public static (ShapeTestHandle handle, Vector3 probeStartPosition, Vector3 probeEndPosition)
            StartTestMouseCursorLOSProbe(IntersectFlags intersectFlags, Entity excludeEntity = null,
                ShapeTestOptions options = ShapeTestOptions.Default)
        {
            var handle = StartTestMouseCursorLOSProbe(out var probeStartPosition, out var probeEndPosition,
                intersectFlags, excludeEntity, options);
