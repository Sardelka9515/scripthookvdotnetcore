...
public static unsafe class Core
{
    ...
    private static delegate* unmanaged<HMODULE, delegate* unmanaged<IntPtr, void>, bool> RegisterScriptFiber = (delegate* unmanaged<HMODULE, delegate* unmanaged<IntPtr, void>, bool>)Import("RegisterScript");
    private static delegate* unmanaged<IntPtr, void> ScheduleCallback = (delegate* unmanaged<IntPtr, void>)Import("ScheduleCallback");
    ...
    {
        lock (_scripts) { return new(_scripts); }
    }
    ...
    public static void RequestScriptCreation(Script script, Action<bool,Script> callback = null)
    {
        ScheduleCallback(Marshal.GetFunctionPointerForDelegate(() =>
        {
            try
            {
                if(!RegisterScriptFiber(CurrentModule, (delegate* unmanaged<IntPtr, void>)Marshal.GetFunctionPointerForDelegate(script.ScriptEntry)))
                {
                    throw new ArgumentException($"An invalid module is supplied. Module: {CurrentModule}. Make sure you set SHVDN.Core.CurrentModule properly during module initialization");
                }
                lock (_scripts)
                {
                    _scripts.Add(script);
