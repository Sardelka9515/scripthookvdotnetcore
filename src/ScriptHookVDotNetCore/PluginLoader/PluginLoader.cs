...
namespace McMaster.NETCore.Plugins
{
    ...
    public class PluginLoader : IDisposable
    {
        ...
        // to inline it into the caller method. That could then prevent successful unloading
        ...
        private void Unload(out WeakReference ctxWeakRef)
        {
            if (!IsUnloadable)
            {
        ...
        private void Unload(int milliSecondsTimeout = 5000)
        {
            Unload(out var ctxRef);
            if (milliSecondsTimeout > 0)
            {
                var cycles = milliSecondsTimeout / 10;
                for (int i = 0; i < cycles && ctxRef.IsAlive; i++)
                {
                    GC.Collect();
