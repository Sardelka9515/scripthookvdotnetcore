...
namespace GTA.UI
{
    public class TextElement : IElement
    {
        ...
        public TextElement(string caption, PointF position, float scale, Color color, Font font, Alignment alignment,
            bool shadow, bool outline, float wrapWidth)
        {
        ...
        {
            get { return Alignment == Alignment.Center; }
            set
            {
                if (value)
                {
        ...
        public float Width
        {
            get
            {
                ...
                return Screen.Width * Call<float>(Hash.END_TEXT_COMMAND_GET_SCREEN_WIDTH_OF_DISPLAY_TEXT, 1);
            }
        }
        ...
        public float ScaledWidth
        {
            get
            {
                ...
                return Screen.ScaledWidth * Call<float>(Hash.END_TEXT_COMMAND_GET_SCREEN_WIDTH_OF_DISPLAY_TEXT, 1);
            }
        }
        ...
        void InternalDraw(SizeF offset, float screenWidth, float screenHeight)
        {
            ...
            if (Shadow)
            {
                Call(Hash.SET_TEXT_DROP_SHADOW);
            }

            if (Outline)
            {
                Call(Hash.SET_TEXT_OUTLINE);
            }

            Call(Hash.SET_TEXT_FONT, Font);
            Call(Hash.SET_TEXT_SCALE, Scale, Scale);
            Call(Hash.SET_TEXT_COLOUR, Color.R, Color.G, Color.B, Color.A);
            Call(Hash.SET_TEXT_JUSTIFICATION, Alignment);
            ...
            if (WrapWidth > 0.0f)
            {
                switch (Alignment)
                {
                    case Alignment.Center:
                        Call(Hash.SET_TEXT_WRAP, x - (w / 2), x + (w / 2));
                        break;
                    case Alignment.Left:
                        Call(Hash.SET_TEXT_WRAP, x, x + w);
                        break;
                    case Alignment.Right:
                        Call(Hash.SET_TEXT_WRAP, x - w, x);
                        break;
                }
            }
            else if (Alignment == Alignment.Right)
            {
                Call(Hash.SET_TEXT_WRAP, 0.0f, x);
            }

            Function.Call(Hash.BEGIN_TEXT_COMMAND_DISPLAY_TEXT, CellEmailBcon);
            ...
            Call(Hash.END_TEXT_COMMAND_DISPLAY_TEXT, x, y);
        }
    }
}