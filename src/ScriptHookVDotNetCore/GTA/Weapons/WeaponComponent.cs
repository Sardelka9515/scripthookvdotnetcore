    ...
    public sealed class WeaponComponent
    {
        ...
        public bool Active
        {
            get
            {
                ...
                return Call<bool>(Hash.HAS_PED_GOT_WEAPON_COMPONENT, _owner.Handle, _weapon.Hash, ComponentHash);
            }
            set
            {
                if (_weapon == null || _owner == null || !_owner.Exists())
                ...
                Call(value ? Hash.GIVE_WEAPON_COMPONENT_TO_PED : Hash.REMOVE_WEAPON_COMPONENT_FROM_PED, _owner.Handle,
                    _weapon.Hash, ComponentHash);
            }
        }

        public string DisplayName =>
            _weapon != null ? GetComponentDisplayNameFromHash(_weapon.Hash, ComponentHash) : string.Empty;
        ...
        static string GetComponentDisplayNameFromHash(WeaponHash hash, WeaponComponentHash component)
        {
            ...
            for (int i = 0, count = Call<int>(Hash.GET_NUM_DLC_WEAPONS); i < count; i++)
            {
                unsafe
                {
                    DlcWeaponData weaponData;
                    if (Call<bool>(Hash.GET_DLC_WEAPON_DATA, i, &weaponData))
                    {
                        if (weaponData.Hash == hash)
                        {
                            int maxComp = Call<int>(Hash.GET_NUM_DLC_WEAPON_COMPONENTS, i);

                            for (int j = 0; j < maxComp; j++)
                            {
                                DlcWeaponComponentData componentData;
                                if (Call<bool>(Hash.GET_DLC_WEAPON_COMPONENT_DATA, i, j, &componentData))
                                {
        ...
        {
            return (WeaponAttachmentPoint)NativeMemory.GetAttachmentPointHash((uint)hash, (uint)componentHash);
        }

        public static WeaponComponentHash[] GetAllHashes()
        {
            return NativeMemory.GetAllWeaponComponentHashes().Select(x => (WeaponComponentHash)x).ToArray();
        }
    }
}