...
namespace GTA
{
    public sealed class VehicleWheelCollection : IEnumerable<VehicleWheel>, IEnumerable
    {
        ...
        [Obsolete(
            "VehicleWheel indexer overload with int index does not support any of the wheels wheel_lm2, wheel_rm2, wheel_lm3, or wheel_lm3, but provided for legacy scripts compatibility in v3 API. Use VehicleWheel indexer overload with VehicleWheelBoneId enum instead.")]
        public VehicleWheel this[int index]
        {
            get
            {
                ...
                if (index < 6)
                {
                    ...
                    return _vehicleWheels[boneIndexZeroBased] ??
                           (_vehicleWheels[boneIndexZeroBased] = new VehicleWheel(Vehicle, index));
                }
                ...
                else
                {
                    ...
                    return _vehicleWheelsForIndex6And7[index - 6] ??
                           (_vehicleWheelsForIndex6And7[index - 6] = new VehicleWheel(Vehicle, index));
                }
            }
        }
        ...
        public VehicleWheel this[VehicleWheelBoneId boneId]
        {
            get
            {
                ...
                int boneIndexZeroBased = (int)boneId - 11;
                return _vehicleWheels[boneIndexZeroBased] ??
                       (_vehicleWheels[boneIndexZeroBased] = new VehicleWheel(Vehicle, boneId));
            }
        }
        ...
        public VehicleWheel GetWheelByIndexOfCollection(int index)
        {
            ...
            var wheelAddr = NativeMemory.GetVehicleWheelAddressByIndexOfWheelArray(vehicleAddr, index);
            if (wheelAddr == IntPtr.Zero)
                return null;

            var boneId = (VehicleWheelBoneId)NativeMemory.ReadInt32(wheelAddr + NativeMemory.VehicleWheelIdOffset);
            int boneIndexZeroBased = (int)boneId - 11;
            return _vehicleWheels[boneIndexZeroBased] ??
                   (_vehicleWheels[boneIndexZeroBased] = new VehicleWheel(Vehicle, boneId, wheelAddr));
        }
        ...
        public IEnumerator<VehicleWheel> GetEnumerator()
        {
            ...
            for (int i = 0; i < wheelCount; i++)
            {
                ...
                var wheelAddress = NativeMemory.GetVehicleWheelAddressByIndexOfWheelArray(vehicleAddr, i);

                var boneId =
                    (VehicleWheelBoneId)NativeMemory.ReadInt32(wheelAddress + NativeMemory.VehicleWheelIdOffset);
                int boneIndexZeroBased = (int)boneId - 11;
                yield return _vehicleWheels[boneIndexZeroBased] ?? (_vehicleWheels[boneIndexZeroBased] =
                    new VehicleWheel(Vehicle, boneId, wheelAddress));
            }
        }
        ...
        public int Count
        {
            get
            {
                ...
                if (address == IntPtr.Zero || NativeMemory.WheelCountOffset == 0)
                {
                    return 0;
                }

                return NativeMemory.ReadInt32(address + NativeMemory.WheelCountOffset);
            }
        }
        ...
        public VehicleWheel[] GetAllWheels()
        {
            ...
            if (vehicleAddress == IntPtr.Zero || NativeMemory.WheelCountOffset == 0)
            {
                return Array.Empty<VehicleWheel>();
            }

            var wheelCount = NativeMemory.ReadInt32(vehicleAddress + NativeMemory.WheelCountOffset);
            var returnWheelArray = new VehicleWheel[wheelCount];

            for (int i = 0; i < returnWheelArray.Length; i++)
            {
                var wheelAddress = NativeMemory.GetVehicleWheelAddressByIndexOfWheelArray(vehicleAddress, i);
                var boneId =
                    (VehicleWheelBoneId)NativeMemory.ReadInt32(wheelAddress + NativeMemory.VehicleWheelIdOffset);
