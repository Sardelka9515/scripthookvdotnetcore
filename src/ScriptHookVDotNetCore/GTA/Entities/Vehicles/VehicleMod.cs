...
namespace GTA
{
    public sealed class VehicleMod
    {
        ...
        public int Count => Call<int>(Hash.GET_NUM_VEHICLE_MODS, Vehicle.Handle, Type);

        public int Index
        {
            get => Call<int>(Hash.GET_VEHICLE_MOD, Vehicle.Handle, Type);
            set => Call(Hash.SET_VEHICLE_MOD, Vehicle.Handle, Type, value, Variation);
        }
        ...
        {
            get => Call<bool>(Hash.GET_VEHICLE_MOD_VARIATION, Vehicle.Handle, Type);
            set => Call(Hash.SET_VEHICLE_MOD, Vehicle.Handle, Type, Index, value);
        }
        ...
        public string LocalizedName
        {
            get
            {
                ...
                if (Type == VehicleModType.FrontWheel || Type == VehicleModType.RearWheel)
                {
                    ...
                    if (index >= count / 2)
                    {
                        return Game.GetLocalizedString("CHROME") + " " +
                               Game.GetLocalizedString(Call<string>(Hash.GET_MOD_TEXT_LABEL, Vehicle.Handle, Type,
                                   index));
                    }
                    else
                    {
                        return Game.GetLocalizedString(Call<string>(Hash.GET_MOD_TEXT_LABEL, Vehicle.Handle, Type,
                            index));
                    }
                }
                ...
                if (index > -1)
                {
                    cur = Call<string>(Hash.GET_MOD_TEXT_LABEL, Vehicle.Handle, Type, index);
        ...
        public string LocalizedTypeName
        {
            get
            {
                ...
                switch (Type)
                {
                    ...
                    default:
                        cur = Call<string>(Hash.GET_MOD_SLOT_NAME, Vehicle.Handle, Type);
                        if (!string.IsNullOrEmpty(Game.GetLocalizedString(cur)))
                        {
                            cur = Game.GetLocalizedString(cur);
                        }
